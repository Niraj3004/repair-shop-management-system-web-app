...
export default function AdminLayout() {
  ...
  const navItems = [
    ...
    { name: 'Send Notification', path: '/admin/send-notification', icon: <Send className="w-5 h-5" /> },
    { name: 'API Tester', path: '/admin/api-tester', icon: <Terminal className="w-5 h-5" /> },
  ];
        ...
        <header className="h-16 bg-white border-b flex items-center justify-between px-4 md:px-8 sticky top-0 z-30">
          ...
          <div className="hidden md:block">
            <h1 className="text-xl font-semibold text-slate-800">
              {navItems.find(item => item.path === location.pathname || (item.path !== '/admin' && location.pathname.startsWith(item.path)))?.name || 'Admin Dashboard'}
            </h1>
          </div>
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <div className="flex items-center gap-3 cursor-pointer group hover:bg-slate-50 p-1.5 rounded-full transition-all border border-transparent hover:border-slate-200">
                  <div className="w-8 h-8 rounded-full bg-gradient-to-br from-indigo-500 to-indigo-600 flex items-center justify-center text-white font-bold uppercase shadow-sm">
                    {user?.firstName?.[0]}
                  </div>
                  <div className="hidden lg:block text-left mr-1">
                    <p className="text-sm font-semibold text-slate-700 leading-none">{user?.firstName} {user?.lastName}</p>
                    <p className="text-[10px] text-slate-500 mt-1">System Administrator</p>
                  </div>
                </div>
              </DropdownMenuTrigger>
